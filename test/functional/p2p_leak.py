...
Also test that nodes that send unsupported service bits to pengolincoind are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if pengolincoind ban behavior changes
    def on_open(self):
        super().on_open()
...
class P2PLeakTest(PengolinCoinTestFramework):
