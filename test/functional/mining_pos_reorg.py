...
class ReorgStakeTest(PengolinCoinTestFramework):
    ...
    def check_money_supply(self, expected_pgo):
        # verify that nodes have the expected PGO supply
        pgo_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(pgo_supply, [DecimalAmt(expected_pgo)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that PGO supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check PGO supply...")
