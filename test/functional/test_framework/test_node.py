...
class TestNode():
    """A class for representing a pengolincoind node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any pengolincoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by pengolincoind, and
        # potentially interfere with our attempt to authenticate
        delete_cookie_file(self.datadir)
        self.process = subprocess.Popen(self.args + extra_args, stderr=stderr, *args, **kwargs)
        self.running = True
        self.log.debug("pengolincoind started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the pengolincoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. pengolincoind still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to pengolincoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to pengolincoind
        expected_msg: regex that stderr should match when pengolincoind fails

        Will throw if pengolincoind starts without an error.
        Will throw if an expected_msg is provided and it does not match pengolincoind's stdout."""
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes pengolincoind to shutdown, so this method takes
...
class TestNodeCLI():
    """Interface to pengolincoin-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with pengolincoin-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run pengolincoin-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same pengolincoin-cli call"
