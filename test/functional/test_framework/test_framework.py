...
TMPDIR_PREFIX = "pengolincoin_func_test_"


class PengolinCoinTestFramework():
    """Base class for a pengolincoin test script.

    Individual pengolincoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave pengolincoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop pengolincoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing pengolincoind/pengolincoin-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use pengolincoin-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: pengolincoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a pengolincoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple pengolincoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as pengolincoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in pengolincoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in pengolincoin.conf
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run pengolincoinds:
    ...
    # PENGOLINCOIN Specific TestFramework
    ...
    """
    Create a ProReg tx, which references an 100 PGO UTXO as collateral.
...
'''
PengolinCoinTestFramework extensions
'''
# !TODO: remove after obsoleting legacy system
class PengolinCoinTier2TestFramework(PengolinCoinTestFramework):
