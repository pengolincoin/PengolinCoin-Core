...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PENGOLINCOINParams"
    else
        PARAMS_DIR="$HOME/.pengolincoin-params"
    fi
fi
...
Failed to fetch the PENGOLINCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
PENGOLINCOIN - fetch-params.sh

This script will fetch the PENGOLINCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common PENGOLINCOIN zkSNARK parameters. Note that it is
