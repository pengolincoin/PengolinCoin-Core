...
/**
 * PENGOLINCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZpgoSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
// extern UniValue setzpgoseed(const UniValue& params, bool fHelp);
// extern UniValue getzpgoseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
// extern UniValue searchdzpgo(const UniValue& params, bool fHelp);
// extern UniValue dzpgostate(const UniValue& params, bool fHelp);
