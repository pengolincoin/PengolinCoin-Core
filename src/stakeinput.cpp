...
CZPgoStake::CZPgoStake(const libzerocoin::CoinSpend& spend)
{
...
int CZPgoStake::GetChecksumHeightFromMint()
{
...
int CZPgoStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZPgoStake::GetChecksum()
{
    return nChecksum;
}

// The zPGO block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZPgoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZPgoStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zPGO staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZPgoStake::GetUniqueness()
{
    //The unique identifier for a zPGO is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZPgoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZPgoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zPGO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPGOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPGO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPGO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPGOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPGO output", __func__);
...
bool CZPgoStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZPgoStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzPGOTracker* zpgoTracker = pwallet->zpgoTracker.get();
    CMintMeta meta;
    if (!zpgoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpgoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!PGO Stake
bool CPgoStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CPgoStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CPgoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CPgoStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CPgoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CPgoStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CPgoStake::GetUniqueness()
{
    //The unique identifier for a PGO stake is the outpoint
...
CBlockIndex* CPgoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
