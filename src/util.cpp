...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "pengolincoin" is a composite category enabling all PENGOLINCOIN-related debug output
            if (ptrCategory->count(std::string("pengolincoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "pengolincoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override pengolincoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "pengolincoind.pid"));
