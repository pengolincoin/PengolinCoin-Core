...
/** Fees smaller than this (in upgo) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k PGO
    const bool isTestnet = Params().IsTestnet();
    if (isTestnet && nHeight < 1001 && nHeight > 1) {
        return 1000 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight <= 700000)
        ...
        ret = blockValue * 60 / 100; //60% of block value.
...
{
    util::ThreadRename("pengolincoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!IsBlockValueValid(pindex->nHeight, nExpectedMint, nMint, nBudgetAmt)) {
        return state.DoS(100, error("%s: reward pays too much (actual=%s vs limit=%s) at height %d",
                                    __func__, FormatMoney(nMint), FormatMoney(nExpectedMint), pindex->nHeight),
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        CAmount nMasternodeValue = GetMasternodePayment(nHeight, nBlockValue);
        if (lastOut.nValue == nMasternodeValue)
            return true;
        ...
        if (!tx->vout[1].IsZerocoinMint()) {
            ...
            if (nHeight != 0 && !IsInitialBlockDownload()) {
                ...
                if (!Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V5_2) && !Params().IsTestnet()) {
                    devScriptPubKey = CScript()
                                      << ParseHex(Params().GetConsensus().strDevpubkey.c_str())
                                      << OP_CHECKSIG;
                } else {
                    devScriptPubKey = CScript() << OP_DUP << OP_HASH160
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nPengolinCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nPengolinCoinBadBlockBits)) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZPGOInputs && in.IsZerocoinSpend())
                ...
                for (const CTxIn& zPgoInput : zPGOInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpgo");
...
int ActiveProtocol()
{
    // SPORK_14 is used for 30001 (v3.0.1)
    if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 30000 (v3.0.0), commented out now.
