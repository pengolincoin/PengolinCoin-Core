...
const char * const PENGOLINCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// PENGOLINCOIN only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pengolincoind -foo=bar
...
#else
    const char* pszModule = "pengolincoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\PENGOLINCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\PENGOLINCOIN
// Mac: ~/Library/Application Support/PENGOLINCOIN
// Unix: ~/.pengolincoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PENGOLINCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PENGOLINCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PENGOLINCOINParams
    // Mac: ~/Library/Application Support/PENGOLINCOINParams
    // Unix: ~/.pengolincoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PENGOLINCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "PENGOLINCOINParams";
#else
    // Unix
    return pathRet / ".pengolincoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/pengolincoin/sapling-spend.params";
        sapling_output = "/usr/share/pengolincoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/pengolincoin/sapling-spend.params";
            sapling_output = "/usr/local/share/pengolincoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", PENGOLINCOIN_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", PENGOLINCOIN_PID_FILENAME));
