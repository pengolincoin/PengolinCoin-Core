...
#include "qt/pengolincoin/dashboardwidget.h"
#include "qt/pengolincoin/forms/ui_dashboardwidget.h"
#include "qt/pengolincoin/sendconfirmdialog.h"
#include "qt/pengolincoin/txrow.h"
...
DashboardWidget::DashboardWidget(PENGOLINCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of PGO staked."));  // Amount of PGO and zPGO staked.
    ...
    // ui->labelPgo->setVisible(false);
    // ui->labelZpgo->setVisible(false);
    ...
    ui->labelAmountZpgo->setText("0 zPGO");
    ui->labelAmountPgo->setText("0 PGO");
    ...
    ui->btnHowTo->setText(tr("How to get PGO or zPGO"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("PGO")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPgo = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZPGO;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair PGO, zPGO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            pgo = (pair.first != 0) ? pair.first / 100000000 : 0;
            zpgo = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPgo += pair.first;
            chartData->totalZpgo += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPgo.append(pgo);
        chartData->valueszPgo.append(zpgo);

        int max = std::max(pgo, zpgo);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("PGO");
    set1 = new QBarSet("zPGO");
    ...
    set0->append(chartData->valuesPgo);
    set1->append(chartData->valueszPgo);
