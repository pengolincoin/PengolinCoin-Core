...
#include "qt/pengolincoin/forms/ui_addresseswidget.h"
#include "qt/pengolincoin/addresslabelrow.h"
#include "qt/pengolincoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(PENGOLINCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a PENGOLINCOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress pgoAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(pgoAdd.Get(), label.toUtf8().constData(),
                pgoAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
