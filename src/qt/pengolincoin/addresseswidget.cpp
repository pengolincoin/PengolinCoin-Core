...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto pgoAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(pgoAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(pgoAdd);
        if (walletModel->updateAddressBookLabels(pgoAdd, label.toUtf8().constData(),
