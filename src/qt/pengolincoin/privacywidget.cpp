...
#include "qt/pengolincoin/forms/ui_privacywidget.h"
#include "qt/pengolincoin/qtutils.h"
#include "guiutil.h"
#include "qt/pengolincoin/denomgenerationdialog.h"
#include "qt/pengolincoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(PENGOLINCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zPGO anonymizes your PGO by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zPGO or convert back to PGO"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 PGO ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zPGO");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zPGO");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zPGO");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zPGO");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zPGO");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zPGO");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zPGO");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zPGO");
    ...
    // Only Convert to PGO enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzPGO->setTitleClassAndText("btn-title-grey", "Total 0 zPGO");
    ui->btnTotalzPGO->setSubTitleClassAndText("text-subtitle", "Show denominations of zPGO owned.");
    ui->btnTotalzPGO->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select PGO outputs to mint into zPGO.");
    ...
    connect(ui->btnTotalzPGO, SIGNAL(clicked()), this, SLOT(onTotalZpgoClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zPGO");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of PGO to mint into zPGO"));
    }else{
        btnText = tr("Convert back to PGO");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zPGO to convert back into PGO"));
    }
...
void PrivacyWidget::onTotalZpgoClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzPGO->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzPGO->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zPGO is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zPGO, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zPGO").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zPGO minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpgo(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zPGO converted back to PGO"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPGO </b>";
    ...
    ui->btnTotalzPGO->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
