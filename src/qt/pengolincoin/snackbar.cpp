...
#include "qt/pengolincoin/forms/ui_snackbar.h"
#include "qt/pengolincoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PENGOLINCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PENGOLINCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
