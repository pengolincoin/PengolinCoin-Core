...
SettingsFaqWidget::SettingsFaqWidget(PENGOLINCOINGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendablePGO,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendablePGO,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("PENGOLINCOIN is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("PENGOLINCOIN utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendablePGOContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received PENGOLINCOIN requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your PENGOLINCOIN wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendablePGO->setText(unspendablePGOContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of PENGOLINCOIN with a minimum of 600 confirmations.")) +
    ...
    QString masternodeContent = formatFAQContent(
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in PENGOLINCOIN Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the PENGOLINCOIN network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendablePGO, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendablePGO);});
    ...
    if (parent)
        connect(parent, &PENGOLINCOINGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
