...
#include "qt/pengolincoin/forms/ui_masternodeswidget.h"
#include "qt/pengolincoin/qtutils.h"
#include "qt/pengolincoin/mnrow.h"
#include "qt/pengolincoin/mninfodialog.h"
...
MasterNodesWidget::MasterNodesWidget(PENGOLINCOINGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (Params().MasternodeCollateral())){
            inform(tr("Not enough balance to create a masternode, 100,000 PGO required."));
            return;
        }
        showHideOp(true);
