...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PGO);
    ...
    if (settings.contains("nAnonymizePengolinCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizepengolincoinamount", settings.value("nAnonymizePengolinCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pengolincoin.cpp during shotdown to also remove the window positions
