...
#include "ui_zpgocontroldialog.h"
...
std::set<std::string> ZPgoControlDialog::setSelectedMints;
std::set<CMintMeta> ZPgoControlDialog::setMints;

bool CZPgoControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZPgoControlDialog::COLUMN_DENOMINATION || column == ZPgoControlDialog::COLUMN_VERSION || column == ZPgoControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZPgoControlDialog::ZPgoControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZPgoControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zPGO Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZPgo->setProperty("cssClass", "text-main-purple");
    ui->labelZPgo_int->setProperty("cssClass", "text-main-purple");
...
ZPgoControlDialog::~ZPgoControlDialog()
{
    delete ui;
}

void ZPgoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPgoControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZPgoControlWidgetItem* itemDenom(new CZPgoControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZPgoControlWidgetItem *itemMint = new CZPgoControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zpgoTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpgoTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPGO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPGO seed used to mint this zPGO is not the same as currently hold in the wallet";
            else
...
void ZPgoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPgoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZPgo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZPgoControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZPgoControlDialog::GetSelectedMints()
{
...
void ZPgoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
