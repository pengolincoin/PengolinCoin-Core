...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    CAmount nNet = rec->pgo + rec->debit;
    ...
    // Amount
    //
    if (wtx.IsCoinBase() && rec->pgo == 0) {
        //
        // Coinbase
        //
        CAmount nUnmatured = 0;
        for (const CTxOut& txout : wtx.vout)
            nUnmatured += wallet->GetPgo(txout, ISMINE_ALL);
        strHTML += "<b>" + tr("Pgo") + ":</b> ";
        ...
        // Pgo
        //
        strHTML += "<b>" + tr("Pgo") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut& txout : wtx.vout) {
                ...
                if (toSelf)
                    strHTML += "<b>" + tr("Pgo") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }

            if (fAllToMe) {
                // Payment to self
                CAmount nChange = wtx.GetChange();
                CAmount nValue = rec->pgo - nChange;
                strHTML += "<b>" + tr("Total debit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total pgo") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }
            ...
            for (const CTxOut& txout : wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Pgo") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, wallet->GetPgo(txout, ISMINE_ALL)) + "<br>";
        }
    }
    ...
    // Message from normal pengolincoin:URI (pengolincoin:XyZ...?message=example)
