...
#include "qt/pengolincoin/pengolincoingui.h"
...
{
    return QCoreApplication::translate("pengolincoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in pengolincoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in pengolincoin.qrc)
...
/** Class encapsulating PENGOLINCOIN Core startup and shutdown.
...
/** Main PENGOLINCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (PENGOLINCOINGUI)
...
private:
    ...
    PENGOLINCOINGUI* window{nullptr};
...
{
    window = new PENGOLINCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PENGOLINCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PENGOLINCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(PENGOLINCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PENGOLINCOINGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // PENGOLINCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PENGOLINCOINGUI::handlePaymentRequest);
        connect(window, &PENGOLINCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. PENGOLINCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(pengolincoin_locale);
    Q_INIT_RESOURCE(pengolincoin);
    ...
    // pengolincoin: links repeatedly have their payment requests routed to this process:
