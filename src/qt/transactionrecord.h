...
public:
    ...
    TransactionRecord(unsigned int size) : hash(), time(0), type(Other), address(""), debit(0), pgo(0), size(size), idx(0)
    {
    }

    TransactionRecord(uint256 hash, qint64 time, unsigned int size) : hash(hash), time(time), type(Other), address(""), debit(0),
                                                   pgo(0), size(size), idx(0)
    {
    }

    TransactionRecord(uint256 hash, qint64 time, unsigned int size, Type type, const std::string& address, const CAmount& debit, const CAmount& pgo) : hash(hash), time(time), type(type), address(address), debit(debit), pgo(pgo),
                                                                                                                                       size(size), idx(0)
    {
    }
