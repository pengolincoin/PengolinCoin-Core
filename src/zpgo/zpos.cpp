...
bool CLegacyZPgoStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZPgoStake from non zc-spend", __func__);
    ...
    *this = CLegacyZPgoStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zpgo stake origin", __func__);

    // All good
    return true;
}

CLegacyZPgoStake::CLegacyZPgoStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZPgoStake::GetIndexFrom() const
{
...
CAmount CLegacyZPgoStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZPgoStake::GetUniqueness() const
{
...
bool CLegacyZPgoStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zPGO stake block: height %d outside range", __func__, nHeight);
