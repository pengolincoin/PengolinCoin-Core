...
CzPGOTracker::CzPGOTracker(std::string strWalletFile)
{
...
CzPGOTracker::~CzPGOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPGOTracker::Init()
{
...
bool CzPGOTracker::Archive(CMintMeta& meta)
{
...
bool CzPGOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPGOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPGOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPGOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPGOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPGOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPGOTracker::GetSerialHashes()
{
...
CAmount CzPGOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPGOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPGOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPGOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPGOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPGOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPGOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPGOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPGOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPGOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPGOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPGOWallet* zPGOWallet)
{
    bool iszPGOWalletInitialized = (NULL != zPGOWallet);
    CMintMeta meta;
    ...
    if (! iszPGOWalletInitialized)
        zPGOWallet = new CzPGOWallet(strWalletFile);
    meta.isSeedCorrect = zPGOWallet->CheckSeed(dMint);
    if (! iszPGOWalletInitialized)
        delete zPGOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPGOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPGOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPGOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPGOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPGOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPGOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPGOWallet* zPGOWallet = new CzPGOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPGOWallet);
        }
        delete zPGOWallet;
        LogPrint("zero", "%s: added %d dzpgo from DB\n", __func__, listDeterministicDB.size());
    }
