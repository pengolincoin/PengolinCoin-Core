...
CzPGOWallet::CzPGOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpgo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPGOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPGOSeed_deprecated()) {
                    LogPrintf("%s: Updated zPGO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpgo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpgo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPGOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPGOCount(nCountLastUsed);
    else if (!walletdb.ReadZPGOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPGOWallet::Lock()
{
    seedMaster = 0;
}

void CzPGOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPGOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPGO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPGOWallet::LoadMintPoolFromDB()
{
...
void CzPGOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPGOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPGOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpgoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPGOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPGO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpgoTracker which also adds to database
    pwalletMain->zpgoTracker->Add(dMint, true);
...
void CzPGOWallet::SeedToZPGO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPGOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPGOWallet::GenerateDeterministicZPGO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPGOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPGOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPGOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
