...
#include "zpgo/zpgowallet.h"
#include "zpgo/zpgotracker.h"
#include "zpgo/deterministicmint.h"
#include <assert.h>
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb)
{
    ...
    if (fFromLoadWallet) {
        ...
        // Break debit/pgo balance caches:
        wtx.MarkDirty();
...
{
    return zpgoTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetUnspentPgo(const isminefilter& filter) const
{
    ...
    CAmount pgo = 0;
    if (filter & ISMINE_SPENDABLE) {
        pgo += pwallet->GetPgo(*this, ISMINE_SPENDABLE, true);
    }
    if (filter & ISMINE_WATCH_ONLY) {
        pgo += pwallet->GetPgo(*this, ISMINE_WATCH_ONLY, true);
    }
    if (filter & ISMINE_COLD) {
        pgo += pwallet->GetPgo(*this, ISMINE_COLD, true);
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        pgo += pwallet->GetPgo(*this, ISMINE_SPENDABLE_DELEGATED, true);
    }
    return pgo;
}

CAmount CWalletTx::GetPgo(const isminefilter& filter) const
{
    CAmount pgo = 0;
    if (filter & ISMINE_SPENDABLE) {
        ...
        if (fPgoCached)
            ...
            nPgoCached = pwallet->GetPgo(*this, ISMINE_SPENDABLE, false);
            fPgoCached = true;
            pgo += nPgoCached;
        }
    }
    if (filter & ISMINE_WATCH_ONLY) {
        if (fWatchPgoCached)
            ...
            nWatchPgoCached = pwallet->GetPgo(*this, ISMINE_WATCH_ONLY, false);
            fWatchPgoCached = true;
            pgo += nWatchPgoCached;
        }
    }
    if (filter & ISMINE_COLD) {
        if (fColdPgoCached)
            ...
            nColdPgoCached = pwallet->GetPgo(*this, ISMINE_COLD, false);
            fColdPgoCached = true;
            pgo += nColdPgoCached;
        }
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        if (fDelegatedPgoCached)
            pgo += nDelegatedPgoCached;
        else {
            nDelegatedPgoCached = pwallet->GetPgo(*this, ISMINE_SPENDABLE_DELEGATED, false);
            fDelegatedPgoCached = true;
            pgo += nDelegatedPgoCached;
        }
    }
    return pgo;
...
CAmount CWalletTx::GetImmaturePgo(bool fUseCache, const isminefilter& filter) const
{
    LOCK(cs_main);
    if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmaturePgoCached && filter == ISMINE_SPENDABLE_ALL)
            return nImmaturePgoCached;
        nImmaturePgoCached = pwallet->GetPgo(*this, filter);
        fImmaturePgoCached = true;
        return nImmaturePgoCached;
    }
...
CAmount CWalletTx::GetAvailablePgo(bool fUseCache) const
{
    return GetUnspentPgo(ISMINE_SPENDABLE_ALL);
}

CAmount CWalletTx::GetColdStakingPgo(bool fUseCache) const
{
    return GetUnspentPgo(ISMINE_COLD);
}

CAmount CWalletTx::GetStakeDelegationPgo(bool fUseCache) const
{
    return GetUnspentPgo(ISMINE_SPENDABLE_DELEGATED);
}

CAmount CWalletTx::UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fPgo) const
{
    if (pwallet == 0)
        return 0;
    ...
    amountToUpdate = (fPgo) ? GetPgo(mimeType) : GetDebit(mimeType);
    cacheFlagToUpdate = true;
    return amountToUpdate;
}

// Return sum of unlocked coins
CAmount CWalletTx::GetUnlockedPgo() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        nPgo += pwallet->GetPgo(txout, ISMINE_SPENDABLE);
        if (!MoneyRange(nPgo))
            throw std::runtime_error("CWalletTx::GetUnlockedPgo() : value out of range");
    }
...
CAmount CWalletTx::GetLockedPgo() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add delegated coins
        nPgo += pwallet->GetPgo(txout, ISMINE_SPENDABLE_DELEGATED);
        ...
        if (!MoneyRange(nPgo))
            throw std::runtime_error("CWalletTx::GetLockedPgo() : value out of range");
    }

    return nPgo;
}

CAmount CWalletTx::GetImmatureWatchOnlyPgo(const bool& fUseCache) const
{
    LOCK(cs_main);
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureWatchPgoCached)
            return nImmatureWatchPgoCached;
        nImmatureWatchPgoCached = pwallet->GetPgo(*this, ISMINE_WATCH_ONLY);
        fImmatureWatchPgoCached = true;
        return nImmatureWatchPgoCached;
    }
...
CAmount CWalletTx::GetAvailableWatchOnlyPgo(const bool& fUseCache) const
{
    if (pwallet == 0)
        return 0;
    ...
    if (fUseCache && fAvailableWatchPgoCached)
        return nAvailableWatchPgoCached;

    CAmount nPgo = 0;
    for (unsigned int i = 0; i < vout.size(); i++) {
        if (!pwallet->IsSpent(GetHash(), i)) {
            const CTxOut& txout = vout[i];
            nPgo += pwallet->GetPgo(txout, ISMINE_WATCH_ONLY);
            if (!MoneyRange(nPgo))
                throw std::runtime_error("CWalletTx::GetAvailablePgo() : value out of range");
        }
    }

    nAvailableWatchPgoCached = nPgo;
    fAvailableWatchPgoCached = true;
    return nPgo;
}

CAmount CWalletTx::GetLockedWatchOnlyPgo() const
{
    if (pwallet == 0)
        return 0;
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZPGO = GetBoolArg("-zapwallettxes", false);
    if (fCheckZPGO)
        zpgoTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zpgo
            if (fCheckZPGO && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetColdStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
            nTotal += pcoin.GetColdStakingPgo();
    });
}
...
CAmount CWallet::GetDelegatedBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
                nTotal += pcoin.GetStakeDelegationPgo();
    });
}
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zPGO in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zpgoTracker->GetMints(true);
    ...
    return zpgoTracker->GetBalance(false, false);
}
...
{
    return zpgoTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zpgoTracker->ListMints(true, true, true);
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmaturePgo(false);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmaturePgo(false, ISMINE_COLD);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmaturePgo(false, ISMINE_SPENDABLE_DELEGATED);
    });
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted())
                nTotal += pcoin.GetAvailableWatchOnlyPgo();
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureWatchOnlyPgo();
    });
}
...
CAmount CWallet::GetLockedWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted() && pcoin.GetDepthInMainChain() > 0)
                nTotal += pcoin.GetLockedWatchOnlyPgo();
    });
}
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-pgostake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CPgoStake> input(new CPgoStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zPGO Staking
    //zPGO
    if ((GetBoolArg("-zpgostake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zPGO set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zpgoTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZpgoBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 PGO collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 PGO collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 100000 PGO.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 100000 PGO.");
                    } else {
                ...
                for (PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setCoins) {
                    ...
                    CAmount nPgo = pcoin.first->vout[pcoin.second].nValue;
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-pengolincoin-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            if (!stakeInput->CreateTxOuts(this, vout, nPgo)) {
            ...
            CAmount nMinFee = 0;
            if (!stakeInput->IsZPGO()) {
                // Set output amount
                int outputs = txNew.vout.size() - 1;
                CAmount nRemaining = nPgo - nMinFee;
                if (outputs > 1) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZPGO());

            {
                TRY_LOCK(zpgoTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
            ...
            //Mark mints as spent
            if (stakeInput->IsZPGO()) {
                CZPgoStake* z = (CZPgoStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zPGO. Current percentage of zPGO: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zPGO are integers > 0, so we can't mint 10% of 9 PGO
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zPGO\n", nBalance);
        return;
    }

    // Percentage of zPGO we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zPGO (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zPGO amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zPGO amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zPGO We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough PGO to mint this denomination
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            nAmount = ((out.tx->GetPgo(filter) - out.tx->GetDebit(filter)) * vMultiSend[i].second) / 100;
...
bool CWallet::CreateZPGOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZPGO(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZPGOOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zpgo output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Pgo
    ...
    // Sign if these are pengolincoin outputs - NOTE that zPGO outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZPGO_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zPGO
        receipt.SetStatus(_("The coin spend has been used"), ZPGO_SPENT_USED_ZPGO);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zpgoTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zpgoTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zpgoTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZPGO_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zpgoTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zpgoTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zpgoTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZPGO_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zPGO privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZPGO_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZPGO_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZPGO_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZPGO_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZPGO_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZPGO_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZPGO_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZPGOModule::createInput(in, mint, hashTxOut, spendVersion)) {
                receipt.SetStatus(_("Cannot create public spend input"), ZPGO_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZPGO_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zPGO"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZPGO_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zPGO public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zPGO used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zpgoTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zPGO mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to pengolincoin address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZPGO_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZPGO_SPEND_OKAY); // Everything okay
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zpgoTracker->ListMints(false, false, true);
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zpgoTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zpgoTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zpgoTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zpgoTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzpgoAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzpgoAuto ? "-autozpgobackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZPgoBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozpgobackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozpgobackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozpgobackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zpgobackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zpgobackuppath", ""));
...
)
{
    ...
    int nStatus = ZPGO_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZPGO_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZPGO_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zpgoTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZPGO_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZPGO_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zpgoTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zpgoTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZPGO_SPEND_OKAY);  // When we reach this point spending zPGO was successful
...
{
    CMintMeta meta;
    if (!zpgoTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zpgoTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zpgoTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zpgoTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zpgoTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zpgoTracker->Get(hashSerial);
    zpgoTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
{
    // We don't even need to worry about this code.. no zPGO.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("pengolincoin-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zpgoTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CzPGOWallet* zwallet)
{
    zwalletMain = zwallet;
    zpgoTracker = std::unique_ptr<CzPGOTracker>(new CzPGOTracker(strWalletFile));
}

CzPGOWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZPgoAutoBackups(bool fEnabled)
{
...
CAmount CWallet::GetPgo(const CTxOut& txout, const isminefilter& filter) const
{
    if (!MoneyRange(txout.nValue))
        throw std::runtime_error("CWallet::GetPgo() : value out of range");
...
CAmount CWallet::GetPgo(const CTransaction& tx, const isminefilter& filter, const bool fUnspent) const
{
    CAmount nPgo = 0;
    for (int i = 0; i < tx.vout.size(); i++) {
        if (fUnspent && IsSpent(tx.GetHash(), i)) continue;
        nPgo += GetPgo(tx.vout[i], filter);
    }
    if (!MoneyRange(nPgo))
        throw std::runtime_error("CWallet::GetPgo() : value out of range");
    return nPgo;
...
void CWalletTx::Init(const CWallet* pwalletIn)
{
    ...
    fDebitCached = false;
    fPgoCached = false;
    fImmaturePgoCached = false;
    fAvailablePgoCached = false;
    fAnonymizablePgoCached = false;
    fAnonymizedPgoCached = false;
    fDenomUnconfPgoCached = false;
    fDenomConfPgoCached = false;
    fWatchDebitCached = false;
    fWatchPgoCached = false;
    fImmatureWatchPgoCached = false;
    fAvailableWatchPgoCached = false;
    fChangeCached = false;
    fColdDebitCached = false;
    fColdPgoCached = false;
    fDelegatedDebitCached = false;
    fDelegatedPgoCached = false;
    fStakeDelegationVoided = false;
    nDebitCached = 0;
    nPgoCached = 0;
    nImmaturePgoCached = 0;
    nAvailablePgoCached = 0;
    nAnonymizablePgoCached = 0;
    nAnonymizedPgoCached = 0;
    nDenomUnconfPgoCached = 0;
    nDenomConfPgoCached = 0;
    nWatchDebitCached = 0;
    nWatchPgoCached = 0;
    nAvailableWatchPgoCached = 0;
    nImmatureWatchPgoCached = 0;
    nColdDebitCached = 0;
    nColdPgoCached = 0;
    nDelegatedDebitCached = 0;
    nDelegatedPgoCached = 0;
    nChangeCached = 0;
    nOrderPos = -1;
}
