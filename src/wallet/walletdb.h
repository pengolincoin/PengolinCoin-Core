...
public:
    ...
    CAmount GetAccountPgoDebit(const std::string& strAccount);
    void ListAccountPgoDebit(const std::string& strAccount, std::list<CAccountingEntry>& acentries);
    ...
    bool WriteZPGOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZPGOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZPGOSeed_deprecated(uint256& seed);
    bool EraseZPGOSeed();
    bool EraseZPGOSeed_deprecated();

    bool WriteZPGOCount(const uint32_t& nCount);
    bool ReadZPGOCount(uint32_t& nCount);
