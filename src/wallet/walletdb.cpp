...
CAmount CWalletDB::GetAccountPgoDebit(const std::string& strAccount)
{
    std::list<CAccountingEntry> entries;
    ListAccountPgoDebit(strAccount, entries);

    CAmount nPgoDebit = 0;
    for (const CAccountingEntry& entry : entries)
        nPgoDebit += entry.nPgoDebit;

    return nPgoDebit;
}

void CWalletDB::ListAccountPgoDebit(const std::string& strAccount, std::list<CAccountingEntry>& entries)
{
    bool fAllAccounts = (strAccount == "*");

    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw std::runtime_error("CWalletDB::ListAccountPgoDebit() : cannot create DB cursor");
    unsigned int fFlags = DB_SET_RANGE;
    while (true) {
        // Read next record
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        ...
        if (ret == DB_NOTFOUND)
            ...
            pcursor->close();
            throw std::runtime_error("CWalletDB::ListAccountPgoDebit() : error scanning DB");
        }
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    ListAccountPgoDebit("*", pwallet->laccentries);
...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("pengolincoin-wallet");
...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(std::make_pair(std::string("dzpgo"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(std::make_pair(std::string("dzpgo"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(std::make_pair(std::string("dzpgo"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(std::make_pair(std::string("dzpgo"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZPGOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZPGOSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZPGOSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZPGOSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZPGOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZPGOSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZPGOCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZPGOCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("dzpgo"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
