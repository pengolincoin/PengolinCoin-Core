...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new PENGOLINCOIN address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be pgoed to 'account'.\n"
...
UniValue delegatoradd(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid() || address.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PENGOLINCOIN address");

    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from PENGOLINCOIN address");
...
UniValue delegatorremove(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get PENGOLINCOIN address from addressBook");
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current PENGOLINCOIN address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"pengolincoinaddress\"   (string) The account pengolincoin address\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new PENGOLINCOIN address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"pengolincoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"pengolincoinaddress\"  (string, required) The pengolincoin address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"pengolincoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"pengolincoinaddress\"  (string, required) The pengolincoin address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"pengolincoinaddress\"  (string) a pengolincoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"pengolincoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"pengolincoinaddress\"  (string, required) The pengolincoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in PGO to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!stakeAddr.IsValid() || !stakeAddr.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PENGOLINCOIN staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!ownerAddr.IsValid() || ownerAddr.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PENGOLINCOIN spending address");
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"pengolincoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"pengolincoinaddress\",     (string) The pengolincoin address\n"
            "      amount,                 (numeric) The amount in PGO\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"pengolincoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"pengolincoinaddress\"  (string, required) The pengolincoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"pengolincoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given pengolincoinaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"pengolincoinaddress\"  (string, required) The pengolincoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in PGO received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in PGO received for this account.\n"
...
CAmount GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter)
{
    ...
    if (strAccount != "*")
        nBalance += walletdb.GetAccountPgoDebit(strAccount);
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of PGO to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 PGO from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 PGO from timotei to akiko with a comment\n" +
    ...
    // Pgo
    CAccountingEntry pgo;
    pgo.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    pgo.strAccount = strTo;
    pgo.nPgoDebit = nAmount;
    pgo.nTime = nNow;
    pgo.strOtherAccount = strFrom;
    pgo.strComment = strComment;
    pwalletMain->AddAccountingEntry(pgo, walletdb);
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"topengolincoinaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nSent an amount from an account to a pengolincoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"topengolincoinaddress\"  (string, required) The pengolincoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in PGO. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 PGO from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The pengolincoin address is the key, the numeric amount in PGO is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!address.IsValid() || address.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid PENGOLINCOIN address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "2. \"keysobject\"   (string, required) A json array of pengolincoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) pengolincoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"pengolincoinaddress\"  (string) A pengolincoin address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in PGO received by the address\n"
...
UniValue listcoldutxos(const UniValue& params, bool fHelp)
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it =
        ...
        if(pcoin->GetColdStakingPgo() == 0 && pcoin->GetStakeDelegationPgo() == 0)
            continue;
...
void AcentryToJSON(const CAccountingEntry& acentry, const std::string& strAccount, UniValue& ret)
{
    ...
    if (fAllAccounts || acentry.strAccount == strAccount) {
        ...
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nPgoDebit)));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"pengolincoinaddress\",    (string) The pengolincoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in PGO. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in PGO. This is negative and only available for the \n"
...
UniValue listaccounts(const UniValue& params, bool fHelp)
{
    ...
    for (const CAccountingEntry& entry : acentries)
        mapAccountBalances[entry.strAccount] += entry.nPgoDebit;
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"pengolincoinaddress\",    (string) The pengolincoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in PGO. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in PGO. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in PGO\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in PGO\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
    ...
    CAmount nPgo = wtx.GetPgo(filter);
    CAmount nDebit = wtx.GetDebit(filter);
    CAmount nNet = nPgo - nDebit;
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending PGOs\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending PGOs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"pengolincoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; pengolincoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending PGOs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in PGO/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total PGO balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the PGO balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the PGO balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in PGO\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in PGO\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in PGO\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in PGO\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,                      (numeric) the transaction fee configuration, set in PGO/kB\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <PENGOLINCOIN Address> <percent>\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PGO address");
...
//         throw std::runtime_error(
//             "getzerocoinbalance\n"
//             "\nReturn the wallet's total zPGO balance.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nResult:\n"
//             "amount         (numeric) Total zPGO balance.\n"
...
//             "\nList all zPGO mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    \"zPGO ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zpgoTracker->ListMints(true, fMatureOnly, true);
...
//             objMint.push_back(Pair("zPGO ID", m.hashPubcoin.GetHex()));     // PubCoin
...
//                     pwalletMain->zpgoTracker->UpdateState(m);
//                 }
//             }
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zpgoTracker->ListMints(true, true, true);
...
//         throw std::runtime_error(
//             "listspentzerocoins\n"
//             "\nList all the spent zPGO mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "\nMint the specified zPGO amount\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. amount      (numeric, required) Enter an amount of Pgo to convert to zPGO\n"
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zPGO minting is DISABLED");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zPGO is currently disabled due to maintenance.");
...
//             "    {\n"
//             "      \"value\": amount,         (numeric) Value in PGO.\n"
//             "      \"address\": \"xxx\"         (string) PGO address or \"zerocoinmint\" for reminted change.\n"
//             "    }\n"
//             "    ,...\n"
//             "  ]\n"
//             "}\n"
...
//     bool fMintChange = params[1].get_bool();        // Mint change to zPGO
//     if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
//         throw JSONRPCError(RPC_WALLET_ERROR, "zPGO minting is DISABLED, cannot mint change");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zPGO old spend only available in regtest for tests purposes");
//     }

//     return DoZpgoSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
// }
...
//             "\nSpend zPGO mints to a PGO address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//     return DoZpgoSpend(nAmount, false, true, vMintsSelected, address_str);
// }


// extern UniValue DoZpgoSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
// {
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzPGOTracker* zpgoTracker = pwalletMain->zpgoTracker.get();
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzPGOTracker* zpgoTracker = pwalletMain->zpgoTracker.get();
//     std::set<CMintMeta> setMints = zpgoTracker->ListMints(false, false, false);
...
//             "2. \"denomination\"         (integer, optional) Export a specific denomination of zPGO\n"

//             "\nResult:\n"
//             "[                   (array of json object)\n"
//             "  {\n"
//             "    \"id\": \"serial hash\",  (string) the mint's zPGO serial hash \n"
...
//             "    \"v\": version,   (numeric) The version of the zPGO\n"
//             "    \"k\": \"privkey\"  (string) The zPGO private key (V2+ zPGO only)\n"
//             "  }\n"
//             "  ,...\n"
//             "]\n"
...
//     CzPGOTracker* zpgoTracker = pwalletMain->zpgoTracker.get();
//     std::set<CMintMeta> setMints = zpgoTracker->ListMints(!fIncludeSpent, false, false);
...
//             "  \"value\": amount    (numeric) The total zPGO value of zerocoin mints that were added\n"
//             "}\n"
...
//         pwalletMain->zpgoTracker->Add(mint, true);
//         count++;
...
//         throw std::runtime_error(
//             "reconsiderzerocoins\n"
//             "\nCheck archived zPGO list to see if any mints were added to the blockchain.\n" +
//             HelpRequiringPassphrase() + "\n"
...
// UniValue setzpgoseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 1)
//         throw std::runtime_error(
//             "setzpgoseed \"seed\"\n"
//             "\nSet the wallet's deterministic zpgo seed to a specific value.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. \"seed\"        (string, required) The deterministic zpgo seed.\n"

//             "\nResult\n"
//             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

//             "\nExamples\n" +
//             HelpExampleCli("setzpgoseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
//             HelpExampleRpc("setzpgoseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
//     CzPGOWallet* zwallet = pwalletMain->getZWallet();
...
// UniValue getzpgoseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || !params.empty())
//         throw std::runtime_error(
//             "getzpgoseed\n"
...
//             "\nResult\n"
//             "\"seed\" : s,  (string) The deterministic zPGO seed.\n"

//             "\nExamples\n" +
//             HelpExampleCli("getzpgoseed", "") + HelpExampleRpc("getzpgoseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zPGO seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zPGO to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zPGO to generate.\n"
    ...
    CzPGOWallet* zwallet = pwalletMain->zwalletMain;
...
// UniValue dzpgostate(const UniValue& params, bool fHelp) {
//     if (fHelp || params.size() != 0)
//         throw std::runtime_error(
//                 "dzpgostate\n"
//                         "\nThe current state of the mintpool of the deterministic zPGO wallet.\n" +
//                 HelpRequiringPassphrase() + "\n"
...
//     obj.push_back(Pair("dzpgo_count", nCount));
...
void static SearchThread(CzPGOWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZPGO(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
// UniValue searchdzpgo(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 3)
//         throw std::runtime_error(
//             "searchdzpgo\n"
//             "\nMake an extended search for deterministically generated zPGO that have not yet been recognized by the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments\n"
//             "1. \"count\"       (numeric) Which sequential zPGO to start with.\n"
//             "2. \"range\"       (numeric) How many zPGO to generate.\n"
//             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

//             "\nExamples\n" +
//             HelpExampleCli("searchdzpgo", "1, 100, 2") + HelpExampleRpc("searchdzpgo", "1, 100, 2"));
...
//     boost::thread_group* dzpgoThreads = new boost::thread_group();
...
//         nPrevThreadEnd = nEnd;
//         dzpgoThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
//     }

//     dzpgoThreads->join_all();

//     zwallet->RemoveMintsFromPool(pwalletMain->zpgoTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) PENGOLINCOIN address for auto minting\n" +
...
//             "5. \"address\"          (string, optional) PENGOLINCOIN address to spend to. If not specified, spend to change add.\n"
...
//         throw std::runtime_error(
//             "clearspendcache\n"
//             "\nClear the pre-computed zPGO spend cache, and database.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             if (fLocked) {
//                 if (zpgoTracker->ClearSpendCache()) {
