...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zpgospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 100000 PGO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPGO send
enum ZerocoinSpendStatus {
    ZPGO_SPEND_OKAY = 0,                            // No error
    ZPGO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPGO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPGO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPGO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPGO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPGO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPGO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPGO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPGO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPGO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPGO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPGO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPGO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPGO_SPENT_USED_ZPGO = 14,                      // Coin has already been spend
    ZPGO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPGO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZPgoBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZPGOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzpgoAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzPGOTracker> zpgoTracker;
    ...
    void setZWallet(CzPGOWallet* zwallet);
    CzPGOWallet* getZWallet();
    bool isZeromintEnabled();
    void setZPgoAutoBackups(bool fEnabled);
    ...
    /// Get 100000 PGO output and keys which can be used for the Masternode
    ...
    CAmount GetPgo(const CTxOut& txout, const isminefilter& filter) const;
    ...
    CAmount GetPgo(const CTransaction& tx, const isminefilter& filter, const bool fUnspent = false) const;
    ...
    /** zPGO reset */
    boost::signals2::signal<void()> NotifyzPGOReset;
...
public:
    ...
    mutable bool fPgoCached;
    mutable bool fImmaturePgoCached;
    mutable bool fAvailablePgoCached;
    mutable bool fAnonymizablePgoCached;
    mutable bool fAnonymizedPgoCached;
    mutable bool fDenomUnconfPgoCached;
    mutable bool fDenomConfPgoCached;
    mutable bool fWatchDebitCached;
    mutable bool fWatchPgoCached;
    mutable bool fImmatureWatchPgoCached;
    mutable bool fAvailableWatchPgoCached;
    mutable bool fChangeCached;
    mutable bool fColdDebitCached;
    mutable bool fColdPgoCached;
    mutable bool fDelegatedDebitCached;
    mutable bool fDelegatedPgoCached;
    mutable bool fStakeDelegationVoided;
    mutable CAmount nDebitCached;
    mutable CAmount nPgoCached;
    mutable CAmount nImmaturePgoCached;
    mutable CAmount nAvailablePgoCached;
    mutable CAmount nAnonymizablePgoCached;
    mutable CAmount nAnonymizedPgoCached;
    mutable CAmount nDenomUnconfPgoCached;
    mutable CAmount nDenomConfPgoCached;
    mutable CAmount nWatchDebitCached;
    mutable CAmount nWatchPgoCached;
    mutable CAmount nImmatureWatchPgoCached;
    mutable CAmount nAvailableWatchPgoCached;
    mutable CAmount nChangeCached;
    mutable CAmount nColdDebitCached;
    mutable CAmount nColdPgoCached;
    mutable CAmount nDelegatedDebitCached;
    mutable CAmount nDelegatedPgoCached;
    ...
    CAmount GetPgo(const isminefilter& filter) const;
    CAmount GetUnspentPgo(const isminefilter& filter) const;
    CAmount GetImmaturePgo(bool fUseCache = true, const isminefilter& filter = ISMINE_SPENDABLE_ALL) const;
    CAmount GetAvailablePgo(bool fUseCache = true) const;
    // Return sum of unlocked coins
    CAmount GetUnlockedPgo() const;
    // Return sum of unlocked coins
    CAmount GetLockedPgo() const;
    CAmount GetImmatureWatchOnlyPgo(const bool& fUseCache = true) const;
    CAmount GetAvailableWatchOnlyPgo(const bool& fUseCache = true) const;
    CAmount GetLockedWatchOnlyPgo() const;
    CAmount GetChange() const;

    // Cold staking contracts pgo/debit
    CAmount GetColdStakingPgo(bool fUseCache = true) const;
    CAmount GetColdStakingDebit(bool fUseCache = true) const;
    CAmount GetStakeDelegationPgo(bool fUseCache = true) const;
    CAmount GetStakeDelegationDebit(bool fUseCache = true) const;

    // Helper method to update the amount and cacheFlag.
    CAmount UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fPgo = true) const;
