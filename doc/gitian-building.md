...
*Setup instructions for a gitian build of PENGOLINCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the PENGOLINCOIN
...
to the PENGOLINCOIN GitHub release page.
...
- [Building PENGOLINCOIN Core](#building-pengolincoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/pengolincoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/pengolincoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/pengolincoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/pengolincoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/pengolincoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/pengolincoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/pengolincoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@pengolincoin.xyz) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/pengolincoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/pengolincoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/pengolincoin-osx-*-build.assert
```
