...
- [Development guidelines](#development-guidelines)
    - [General PENGOLINCOIN Core](#general-pengolincoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PENGOLINCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play PGOs (tPGO)" on the test network, if you
...
PENGOLINCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/pengolincoin-project/pengolincoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_pengolincoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_pengolincoin --log_level=test_suite
$ valgrind -v --leak-check=full src/pengolincoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_pengolincoin.coverage/index.html`.
```

**Sanitizers**
PENGOLINCOIN can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates PGOs (if wallet is enabled).
...
However, in open source software such as PENGOLINCOIN Core, where everyone uses
...
pay attention to for reviewers of PENGOLINCOIN Core code.

General PENGOLINCOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PENGOLINCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against PENGOLINCOIN Core so that they can be integrated
...
file descriptors it will cause problems with PENGOLINCOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:PENGOLINCOIN-Project/PENGOLINCOIN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `pengolincoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `pengolincoin-cli` and the GUI debug console use this table to determine how to
